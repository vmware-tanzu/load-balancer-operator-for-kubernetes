...
import (
	...
	testenv "gitlab.eng.vmware.com/vmware-samples/load-balancer-operator-for-kubernetes/e2e/pkg/env"
)
...
var _ = Describe("AKODeploymentConfig with selector", func() {
	...
	When("one cluster is newly created without the specified label", func() {
		...
		AfterEach(func() {
			...
			By("ensuring Avi Resources is gone")
			testcase.EnsureAviResourcesDeleted(clusterName)
		})
